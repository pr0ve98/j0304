		...
		for(int i=1; i<=no.length-1; i++) {
			for(int j=i+1; j<=no.length; j++) {
				if(rank[i-1] > rank[j-1]) {
					temp = rank[i-1];rank[i-1] = rank[j-1];rank[j-1] = temp; // 순위 변환
					// temp = rank[i-1];
					// rank[i-1] = rank[j-1];
					// rank[j-1] = temp;  와 같은 수식
					temp = no[i-1];no[i-1]=no[j-1];no[j-1]=temp; // 번호 변환
					temp = su[i-1];su[i-1]=su[j-1];su[j-1]=temp; // 점수 변환
				}
			}
		}
